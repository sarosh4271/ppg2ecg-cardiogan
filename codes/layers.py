...
def Dense(units, activation=None):
    op = keras.layers.Dense(units=units, activation=activation, use_bias=True, kernel_initializer=weights_initializer,
...
def Conv1D(filters, kernel_size, strides=1, padding='valid', activation=None, use_bias=True):
    op = keras.layers.Conv2D(filters=filters, kernel_size=(1, kernel_size), strides=(1, strides), padding=padding, data_format='channels_last',
...
def DeConv1D(filters, kernel_size, strides=1, padding='valid', use_bias=True):
    op = keras.layers.Conv2DTranspose(filters= filters, kernel_size=(1, kernel_size), strides=(1, strides), padding=padding, 
...
def BatchNormalization(trainable=True, virtual_batch_size=None):
    op = keras.layers.BatchNormalization(axis=-1, momentum=0.99, epsilon=0.001, center=True, scale=True,
...
def Dropout(rate):
    op = keras.layers.Dropout(rate=rate, noise_shape=None, seed=None)
    
    return op

def flatten():
    op = keras.layers.Flatten(data_format=None)
